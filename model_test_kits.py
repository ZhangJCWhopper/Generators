...
class TimeLineRecorder:
	...
	def save(self, f_name="time_analysis"):
		with open(f_name, 'w') as f:
			json.dump(self._timeline_dict, f)

class PreparePLDA(object):
	"""docstring for PreparePLDA"""
	def __init__(self, parent_dir="/work2/eval", short_ivs_dir="10s_test_male"):
		#wtf
		self.noise_dim = 50

		self.iv_dim = 400
		self.noise_cov = .5
		self.short_ivs_file = os.path.join(parent_dir, "origin", short_ivs_dir)
		if "train" in self.short_ivs_file:
			self.short_ivs_file = os.path.join(self.short_ivs_file, "spk_ivector.ark")
		else:
			self.short_ivs_file = os.path.join(self.short_ivs_file, "ivector.ark")
		self.layer_num = 4
		self.turn = 1
		self.batch_size = 1
		self.target_ark_file = os.path.join(parent_dir, "generated", short_ivs_dir+".ark")
	...
	def generate_samples_light(self, ckpt_dir):
		...
		with tf.Session() as sess:
			with open(self.target_ark_file, "w") as out:
