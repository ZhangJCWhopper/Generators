...
tot_eer=0.0
printf '% 12s' 'EER:'
for condition in $(seq 3); do
  eer=$(awk '{print $3}' $scores | paste - $trials | awk -v c=$condition '{n=9+c; if ($n == "Y") print $1, $4}' | compute-eer - 2>/dev/null)
  tot_eer=$(echo "$tot_eer+$eer" | bc)
  eers[$condition]=$eer
done

eers[0]=$(echo "$tot_eer/3" | bc -l)
